(() => {
    ...
    function rangeInit() {
        ...
        settingSlidersWrapper.forEach((settingSliderWrapper => {
            let settingSlider = settingSliderWrapper.querySelector(".item-setting__slider");
            ...
            let settingSliderValue = settingSliderWrapper.querySelector(".item-setting__value span");
            ...
            if (targetElement.closest(`.button-reset`)) {
                ...
                for (const jug in jugs) if (Object.prototype.hasOwnProperty.call(jugs, jug)) {
                    ...
                    if (jugIndex < values.length) {
                        elementJug.percent = values[jugIndex];
                        jugIndex++;
                    }
                }
                updateSetting();
            ...
            if (targetElement.closest(".button-apply")) {
                ...
                if (totalSliderValue === 100) {
                    ...
                    itemSetting.forEach((sliderWrapper => {
                        ...
                        const specificClass = sliderWrapperClasses.find((className => className.startsWith("item-setting-")));
                        if (specificClass) {
                            const classWithoutPrefix = specificClass.replace("item-setting-", "");
                            jugs[classWithoutPrefix].percent = Number(slider.noUiSlider.get());
                        }
                    }));
                    displayData();
                } else userMessage("error", buttonApply);
            }
        }
    };
    let amount = 0;
