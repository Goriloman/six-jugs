    ...
    if (targetElement.closest(".button-apply")) {
      ...
      if (sum == 100) {
        ...
        settingSlidersWrapper.forEach((settingSliderWrapper) => {
          ...
          const specificClass = classes.find((className) =>
            className.startsWith("item-setting-")
          );

          if (specificClass) {
            // Выводим полный класс
            console.log(specificClass);

            // Извлекаем часть класса после 'item-setting-'
            const classWithoutPrefix = specificClass.replace(
              "item-setting-",
              ""
            );
            console.log(classWithoutPrefix); // выводит 'development' или 'safety' и т.д.
          }
        });
      } else {
